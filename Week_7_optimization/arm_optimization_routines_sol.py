...
def vector_to_goal(arm_with_angles, target):
    ...
    dist = np.array([target[0] - end_pt[0], target[1] - end_pt[1]])
    return dist
...
def do_fmin(angles_start, arm, target):
    ...
    angles_to_target = fmin(func_for_fmin, x0=angles_for_fmin, args=(arm, target))
    output_angles = angles_numpy_to_list(angles_to_target, angles_start[-1][1], angles_start[-1][2])

...
def get_transform_base(base_width=1.0, base_height=0.5):
    ...
    mat_return = mt.make_translation_matrix(0, base_width/4) @ \
        mt.make_rotation_matrix(np.pi/ 2.0) @ \
            mt.make_scale_matrix(base_height/2, base_width/2) @ \
            mat_return
    return mat_return
...
def get_transform_link(arm_length, arm_height):
    ...
    mat_return = np.identity(3)
    mat_return = mt.make_translation_matrix(arm_length/2, 0) @ \
        mt.make_scale_matrix(arm_length/2, arm_height/2) @ mat_return
...
def get_transform_palm(palm_width):
    ...
    mat_return = np.identity(3)
    mat_return = mt.make_scale_matrix(.1 * palm_width/2, palm_width / 2) @ mat_return
...
def get_transform_finger(palm_width, finger_size, b_is_top):
    ...
    mat_return = mt.make_translation_matrix(finger_size[0]/2, 0.5 * palm_width if b_is_top else -0.5 * palm_width) @ \
        mt.make_scale_matrix(finger_size[0]/2,  finger_size[1]/2)
...
def get_matrix_base(base_link):
    ...
    point_in_local = np.array([1.0, 0.0,  1.0])
